...
-- Trigger to auto-create inventory when a variant is created
DELIMITER //
CREATE TRIGGER after_variant_insert
AFTER INSERT ON product_variant
FOR EACH ROW
BEGIN
    INSERT INTO inventory (variant_id, current_stock)
...
BEGIN
    ...
    IF available < NEW.quantity THEN
        ...
        SET MESSAGE_TEXT = 'INSUFFICIENT STOCK FOR VARIANT';
    END IF;
END//
DELIMITER ;
...
BEGIN
    ...
    INSERT INTO inventory_log (variant_id, change_amount, reason, created_at)
...
BEGIN
    IF NEW.status = 'APPROVED' AND OLD.status != 'APPROVED' THEN
        ...
        UPDATE inventory i
        INNER JOIN return_item ri ON i.variant_id = ri.variant_id
